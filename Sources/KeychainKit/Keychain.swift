...
extension Keychain {
  ...
  /// Constructs a base Keychain query dictionary for a fully-qualified account string.
  ///
  /// This overload is used internally for migration and bulk operations where the Security
  /// framework has already returned the fully-qualified account name.
  private func baseQuery(
    account: String,
    sync: KeychainSync?,
    returnData: Bool = false,
    ...
    if let sync = sync {
      ...
      query[kSecAttrSynchronizable as String] = KeychainSync.anyAttributeValue
    }
  ...
  private func load(
    ...
    load(account: key.value, sync: sync)
  }

  /// Loads raw data associated with a fully-qualified Keychain account string.
  private func load(
    account: String,
    sync: KeychainSync? = nil
  ) -> Data? {
    ...
    let status = SecItemCopyMatching(
      baseQuery(account: account, sync: sync, returnData: true) as CFDictionary,
      &result
    )
...
extension Keychain {
  ...
  private func storeData(
    ...
    try storeData(data, account: key.value, sync: sync, accessible: accessible)
  }

  /// Inserts or updates raw data for a fully-qualified Keychain account string.
  private func storeData(
    _ data: Data,
    account: String,
    sync: KeychainSync,
    ...
    var query = baseQuery(account: account, sync: sync)
...
extension Keychain {
  ...
  public func url(
    ...
    if let url: URL = value(for: key, sync: sync) {
      return url
    }

    return string(for: key, sync: sync).flatMap(URL.init(string:))
  }
...
extension Keychain {
  ...
  public func allKeys(sync: KeychainSync? = nil) -> [String] {
    var result: CFTypeRef?
...
extension Keychain {
  ...
  public func remove(
    ...
    remove(account: key.value, sync: sync)
  }

  /// Removes a Keychain item for a fully-qualified account string.
  @discardableResult
  private func remove(
    account: String,
    sync: KeychainSync? = nil
    ...
    let status = SecItemDelete(
      baseQuery(account: account, sync: sync) as CFDictionary
    )
  ...
  @discardableResult
  public func removeAll(sync: KeychainSync? = nil) -> Bool {
...
extension Keychain {
  ...
  public static func migrate(
    ...
    for keyString in keys {
      guard let data = oldKeychain.load(account: keyString) else {
      ...
      oldKeychain.remove(account: keyString)
      migrated.append(keyString)
    }
    ...
    return MigrationResult(
      migrated: migrated,
      failed: failed,
      didModify: mode == .perform && !migrated.isEmpty
    )
  }
}
